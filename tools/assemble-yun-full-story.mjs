...
const characters = [
  ...
  { id: "huaiyi", name: "槐姨", role: "王女的照料者与民间记忆", bio: "总能把宏大方案重新落到具体的人身上。", color: "#d8c4a7" },
  ...
  { id: "prince_li", name: "砾亲王", role: "鹿暝的弟弟与王位继承人", bio: "因为无法接受父亲被遗忘，邀请折页匠绘制永恒遗像。", color: "#d29086" },
  ...
  { id: "merchant_old", name: "老海商", role: "曾为北陆货单盖章的商会会长", bio: "把未知当成商品，如今仍藏着最后一块轻泡晶。", color: "#c5a783" },
...
const extraText = {
  ...
  T3_02: [
    ...
    ["潜灯王子", "姐姐，我把自己的名字从潮汐表、族谱和祭文里一项项刮掉，才让这座图书馆活到今天。别急着替我写回去。"]
  ],
  T3_03: [
  ...
  T3_04: [
    ...
    ["潜灯王子", "戴上它，我的脸会重新出现在每座钟楼。王国也许安心一天，纸会省下找路的工夫。"],
    ["鹿暝", "那我不带你回去。把值班表给我，我替你守一刻钟。"],
    ["潜灯王子", "先学会看第三列。那里写着哪些求救不能回答。"]
  ],
  T3_05: [
  ...
  T3_08: [
    ...
    ["潜灯王子", "没人证明。你只能把货舱装得像家：名字、口音、错账，还有那锅不能吃的汤。以后有人肯嫌弃它，故乡就还剩一点。"],
    ["旁白", "装载表越写越长，机器反而被挤到第二页。风帆卫士看了三遍，没把任何一个‘无用’项目划掉。"]
  ],
  T3_09: [
  ...
  E_TRUE: [
    ...
    ["澄安", "第一页没有‘胜利’两个字。很好，至少没人能拿它去剪彩。"],
    ["译码员", "远航名额、慢灯覆盖区、未覆盖人口，都在。没有一栏被藏到附录。"],
    ["旁白", "很远的地方，三盏无人看守的灯依次熄灭。第四盏还没有亮。"]
  ],
  E_WARNING_ONLY: [
    ["旁白", "警报穿过城市时，人们第一次抬头看见那条正在增长的白。"],
    ["澄安", "学校停课了，车站挤满了人。可屏幕上只有警报等级，没有一个可去的地方。"],
  ...
  E_ESCAPE_FLAWED: [
    ...
    ["旁白", "后来者打开旧报告，终于在修订记录里找到那句被删掉的话：不要沿白痕回头。"]
  ],
  E_BLACKDOMAIN: [
  ...
  E_BLACKDOMAIN_FLAWED: [
    ...
    ["旁白", "控制塔日志最后一页仍写着正确转速。它被盖上‘影响方案通过’的红章，从未送进施工系统。"]
  ],
  E_MISSED: [
    ["旁白", "文学组为童话颁了一枚奖。奖章送入库房时，库房的墙已经薄得能透过它看见另一侧的星。"],
    ["澄安", "评审意见写了八页‘意象优美’，没有一页抄下纸边速度。"],
    ["旁白", "后来的人终于读懂了。后来，已经没有人能够把读懂这件事告诉别人。"]
  ],
  E_CENSOR: [
    ["旁白", "澄安仍在呼喊，可她的声音被压成一条没有振幅的线。"],
    ["译码员", "我把‘低光速’三个字说出口了。监察不必理解童话，只要知道该在哪一秒剪线。"],
...
const arrivalText = {
  ...
  T1_10: [
    ["旁白", "雪阶尽头，砾亲王带着卫队挡住去路。他是鹿暝的弟弟，也是那个因为害怕父亲被忘记、亲手把折页匠请进王宫的人。"]
  ],
  T2_06: [
    ["旁白", "会馆最里面坐着老海商。当年正是他在北陆货单上盖章，把饥潮鳞、轻泡晶与黑膜伞骨写成了三种普通商品。"]
  ],
  T3_02: [
...
function effectHint(effects = [], target) {
  ...
  const decreasedKeys = new Set(effects.filter((effect) => effect.type === "inc" && effect.by < 0).map((effect) => effect.key));
  if (keys.has("heat")) return "监察风险 · 这次表达可能逼近通信中止";
  if (keys.has("clue_flatten")) return "获得线索 · 画页如何定位并压平世界";
  if (keys.has("clue_spread_rate")) return "获得线索 · 纸边速度与撤离窗口";
  if (keys.has("clue_umbrella") || keys.has("clue_slowlight")) return "获得线索 · 慢光黑域的条件与边界";
  if (keys.has("clue_curvature")) return "获得线索 · 轻泡如何改变前方航路";
  if (keys.has("clue_wake")) return "获得线索 · 远航白痕及其长期副作用";
  if (keys.has("clue_coordinate")) return "获得线索 · 文明坐标为何必须隐藏";
  if (keys.has("clue_escape")) return "获得线索 · 远航需要满足的条件";
  if (keys.has("clue_blackdomain")) return "获得线索 · 黑域安全与永久封闭";
  if (keys.has("clue_common_origin")) return "获得线索 · 灾害、远航与防御的共同来源";
  if (keys.has("defense_bias")) return "方案倾向 · 暂时更相信慢光与防御路线";
  if (keys.has("escape_bias")) return "方案倾向 · 暂时更相信远航路线";
  if (keys.has("politics")) return "人物倾向 · 从王位、秩序与公共责任追问";
  if (keys.has("worlds")) return "世界线索 · 追查王国之外发生过什么";
  if (keys.has("trust")) return "人物倾向 · 优先保留讲述者与普通人的立场";
  if (decreasedKeys.has("trust")) return "信息取舍 · 隐瞒副作用会削弱报告可信度";
  if (keys.has("risk")) return "行动倾向 · 以更高风险换取即时结果";
  if (keys.has("science")) return "分析倾向 · 优先验证现象与机制";
...
endings.push(
  makeSourceNode("E_FOURTH_LIGHT", {
    ...
    text: [
      ...
      ["澄安", "回信已发送。附件里没有颂词，只有三份仍允许修改的方案和每次修改的署名。"],
  ...
  makeSourceNode("E_WAKE_UMBRELLA", {
    ...
    text: [
      ...
      ["澄安", "末舰名单没有写‘牺牲者’。每个人签字时填的是值班岗位。"],
      ["译码员", "他们没有逃往群星。他们的任务是把最后一道白痕闭合，然后留在伞外。"],
...
const story = {
  ...
  presentation: {
    ...
    highlightTerms: {
      ...
      "慢光": { tone: "concept", description: "童话中的可观察现象：光、声音与消息在局部区域传播得更慢。" },
      "慢灯": { tone: "concept", description: "让光与信息变慢的局部安全边界。" },
      "低光速黑域": { tone: "concept", description: "现实译码名称：以永久降低局部光速、封闭逃逸能力来降低文明威胁特征。" },
      "黑域": { tone: "concept", description: "低光速黑域的简称；不是拦截攻击的护盾，而是一份不可逆的安全声明。" },
